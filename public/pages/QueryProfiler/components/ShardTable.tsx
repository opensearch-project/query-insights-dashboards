...
export const ShardTable: React.FC<ShardTableProps> = ({
  ...
  const [sortField, setSortField] = useState<'searchTime' | 'aggTime' | 'id'>('searchTime');
  ...
  const sortedShards = useMemo(() => {
    return [...filteredShards].sort((a, b) => {
      if (sortField === 'id') {
        // String comparison for shard IDs
  ...
  const columns = [
    {
      ...
      width: '35%',
      render: (id: string, item: ShardMetrics) => (
        <EuiLink
            ...
            <div style={{ width: '280px' }}>
              ...
              <EuiFormRow label={`Red (>${redThreshold}%)`} fullWidth>
                <EuiRange
                  ...
                  onChange={(e) => {
                    ...
                    onRedThresholdChange?.(Math.max(val, orangeThreshold + 5));
                  }}
                  showValue
                  valuePrepend=">"
                  fullWidth
              ...
              <EuiFormRow label={`Orange (>${orangeThreshold}%)`} fullWidth>
                <EuiRange
                  ...
                  onChange={(e) => {
                    const val = Number(e.currentTarget.value);
                    onOrangeThresholdChange?.(Math.min(val, redThreshold - 5));
                  }}
                  showValue
                  valuePrepend=">"
                  fullWidth
