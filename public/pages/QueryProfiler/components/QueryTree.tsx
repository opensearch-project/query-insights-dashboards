...
import { EuiFlexGroup, EuiFlexItem, EuiTabs, EuiTab, EuiText, EuiButtonIcon } from '@elastic/eui';
import {
  QueryTreeProps,
  QueryProfile,
  AggregationProfile,
...
export const QueryTree: React.FC<QueryTreeProps> = ({
  ...
  useEffect(() => {
    const hasSearchData = queries && queries.length > 0;
    const hasAggData = aggregations && aggregations.length > 0;
    if (!hasSearchData && hasAggData) {
  ...
  const [leftPanelWidth, setLeftPanelWidth] = useState<number>(
    LAYOUT_CONSTANTS.TREE_PANEL_DEFAULT_WIDTH
  );
      ...
      if (collectors && collectors.length > 0) {
        const transformCollector = (
          ...
          return {
            ...
            type: collector.name || 'Collector',
            description: collector.reason || '',
            time_ms: timeMs,
            time_in_nanos: collector.time_in_nanos || 0,
            percentage,
            breakdown: {},
            children: collector.children?.map((child, idx) =>
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      const newWidth = Math.max(
        ...
        Math.min(LAYOUT_CONSTANTS.TREE_PANEL_MAX_WIDTH, e.clientX - containerLeft)
      );
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '4px' }}>
          {processedQueries.length > 0 ? (
            ...
            <EuiText size="s" color="subdued" style={{ padding: '12px' }}>
